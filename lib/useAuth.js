    ...
    return onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
        try {
          ...
          console.error('Failed to set up manager profile:', e);
          setProfileError(e?.message || 'Could not set up your account.');
        }
      }
    });
  }, []);

  return { user, loading: user === undefined, profileReady, profileError };
}
