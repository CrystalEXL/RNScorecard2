...
import { YEAR, computeTotal, entryId } from './scoring';
...
export function useEntriesByYear(year = YEAR) {
...
export async function saveEntry({ nurseId, month, scores, uid }) {
  const total = computeTotal(scores);
  await setDoc(doc(db, 'nurses', nurseId, 'entries', entryId(month)), {
    year: YEAR,
    month: Number(month),
    scores,
    total,
